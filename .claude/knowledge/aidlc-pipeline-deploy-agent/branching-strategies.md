...
`aidlc-bolt complete --merge` owns the RECORD side of the Bolt boundary (forkState merge-back, forkAudit merge-back) and does not call `aidlc-worktree merge`. No shipped surface issues that call either, so the GIT side is run explicitly at the Bolt boundary: `aidlc-worktree merge --slug <slug> --target <branch> --strategy <squash|merge|rebase>`. Re-measure this before relying on it — if a future generation wires the dispatch, this paragraph is the thing that goes stale.
